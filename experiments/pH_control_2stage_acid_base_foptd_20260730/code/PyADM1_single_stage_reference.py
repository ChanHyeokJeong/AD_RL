...
def PI_pH_controller(pH_sp, pH_meas, dt):
    global err_int, u_NaOH, q_NaOH, u_HCl, q_HCl, Kp, Ki
    global q_NaOH_min, q_NaOH_max, q_HCl_min, q_HCl_max, C_NAOH, C_HCL

    err = pH_sp - pH_meas
    err_int_trial = err_int + err * dt

    # PI gains remain in NaOH-flow equivalent units. Convert the signed command
    # to molar equivalents before splitting it between base and acid dosing.
    q_base_equiv_unsat = Kp * err + Ki * err_int_trial
    u_unsat = C_NAOH * q_base_equiv_unsat
    u_min = -C_HCL * q_HCl_max
    u_max = C_NAOH * q_NaOH_max
    u_sat = max(u_min, min(u_unsat, u_max))

    if u_sat == u_unsat:
        err_int = err_int_trial

    if u_sat >= 0.0:
        u_NaOH = u_sat
        q_NaOH = max(q_NaOH_min, u_NaOH / C_NAOH)
        u_HCl = 0.0
        q_HCl = 0.0
    else:
        u_NaOH = 0.0
        q_NaOH = 0.0
        u_HCl = -u_sat
        q_HCl = max(q_HCl_min, u_HCl / C_HCL)
